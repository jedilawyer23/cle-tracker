...
describe('resolveParseDailyLimit', () => {
  ...
  it('uses PARSE_DAILY_LIMIT for both anonymous and authenticated when it is a positive number', () => {
    expect(resolveParseDailyLimit(true, { PARSE_DAILY_LIMIT: '50' })).toBe(50)
    expect(resolveParseDailyLimit(false, { PARSE_DAILY_LIMIT: '50' })).toBe(50)
  })

  it('falls back to the provider default for a non-numeric or non-positive value', () => {
    expect(resolveParseDailyLimit(true, { PARSE_DAILY_LIMIT: 'abc' })).toBe(10)
    expect(resolveParseDailyLimit(false, { PARSE_DAILY_LIMIT: '0' })).toBe(25)
    expect(resolveParseDailyLimit(true, { PARSE_DAILY_LIMIT: '-5' })).toBe(10)
  })
})
