...
describe('CreditForm', () => {
  ...
  it('warns, without blocking save, when the completion date is after today', () => {
    const onSave = vi.fn()
    render(<CreditForm submitLabel="Save credit" onSave={onSave} today="2026-07-10" />)
    fireEvent.change(screen.getByLabelText(/provider/i), { target: { value: 'CEB' } })
    fireEvent.change(screen.getByLabelText(/activity title/i), { target: { value: 'Ethics' } })
    ...
    fireEvent.click(screen.getByRole('button', { name: /save credit/i }))
    expect(onSave).toHaveBeenCalledWith(expect.objectContaining({ completionDate: '2026-08-01' }))
  })

  it('shows no future-date warning when the completion date is today or earlier', () => {
    render(<CreditForm submitLabel="Save credit" onSave={vi.fn()} today="2026-07-10" />)
    fireEvent.change(screen.getByLabelText(/completion date/i), { target: { value: '2026-07-10' } })
    expect(screen.queryByText(/this date is in the future/i)).not.toBeInTheDocument()
  })
